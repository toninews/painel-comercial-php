<?php
namespace Nexa\Widgets\Wrapper;
...
{
    private $decorated;
    private $emptyMessage = 'Nenhum registro encontrado.';
    ...
    private function createEmptyState($tbody)
    {
        $row = new Element('tr');
        $tbody->add($row);

        $cell = new Element('td');
        $cell->colspan = count((array) $this->decorated->getColumns()) + count((array) $this->decorated->getActions());
        $cell->class = 'datagrid-empty';
        $cell->add($this->emptyMessage);
    ...
    public function createItem($tbody, $item)
    {
        ...
        if ($actions)
        {
            ...
            foreach ($actions as $action)
            {
                ...
                $element->class = 'datagrid-action-cell';
                $element->add($link);
